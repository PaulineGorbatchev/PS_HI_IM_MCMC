...
pk_dw = pk_values * np.exp(- g_m * k ** 2) + pk_nw * (1 - np.exp(- g_m * k ** 2))
...
def calculate_PS(d_Aref, H_z_values, H_ref, r_HI_g, k, mi, result_int, h_z_values, z_values, Omega_HI, pk_dw,
    ...
    return P_IM_g, P_gg, P_IM, P_IM_noise, P_IM_g_wt_T, P_IM_IM_wt_T


P_IM_g, P_gg, P_IM, P_IM_noise, P_IM_g_wt_T, P_IM_IM_wt_T = calculate_PS(d_Aref, H_z_values, H_ref, r_HI_g, k, mi, result_int, h_z_values, z_values, Omega_HI, pk_dw,
                     frequency_to_temperature, theta_pb, r, b_g, growth_rate, b_HI, H0, sigma_8, factor)
...
fact = 4 * np.pi ** 2 / (k ** 2 * delta_k * delta_mi * V_survey)
...
plt.errorbar(k_values, P_gg_values, yerr=P_gg_N_values, fmt='.', capsize=0.01, label='P_gg', color='green')

# Plot P_IM with error bars
plt.errorbar(k_values, P_IM_IM_wt_T, yerr=P_IM_noise_values, fmt='.', capsize=0.01, label='P_IM', color='magenta')
plt.xscale('log')
plt.yscale('log')
...
def log_likelihood_IM_IM(theta, data):
    ...
    _, _, model_IM_IM, _, _, _ = calculate_PS(d_Aref, H_z_values, H_ref1, r_HI_g, k1, mi, result_int, h_z_values, z_values,
                                           Omega_HI, pk_dw1,
...
def log_likelihood_IM_g(theta, data):
    ...
    model_IM_g, _, _, _ , _, _= calculate_PS(d_Aref, H_z_values, H_ref1, r_HI_g, k1, mi, result_int, h_z_values, z_values,
                                           Omega_HI, pk_dw1,
                                           frequency_to_temperature, theta_pb, r1, b_g, growth_rate1, b_HI_e, H_0_e,
                                           sigma_8, factor)
    sigma = 0.1  # Assuming a constant observational uncertainty
...
data_IM_g,_, data_IM_IM,_, _, _ = calculate_PS(d_Aref, H_z_values, H_ref, r_HI_g, k, mi, result_int, h_z_values, z_values, Omega_HI, pk_dw,
...
plt.savefig('getdist_triangle_plot_comparison.pdf')
